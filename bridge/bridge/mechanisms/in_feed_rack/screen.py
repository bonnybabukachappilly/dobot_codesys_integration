...
MECHANISM_NAME = 'm_TV_Rack_Screen'
TOOL_NAME = 't_TV_Rack_Screen'

# Frames
TOOL_FRAME = 'f_TV_Rack_Screen'
REF_PART_FRAME = 'f_Ref_TV_Rack_Screen'
TARGET_FRAME = 'f_TV_Rack_Rail_Targets'

# Objects
REF_PART_NAME = 'TVScreen_Ref'

# Targets
TARGETS: dict[str, str] = {
    'corner_lb': 't_rack_screen_lb',
    'corner_lt': 't_rack_screen_lt',
    'corner_rt': 't_rack_screen_rt',
    'corner_rb': 't_rack_screen_rb',
    'active': 't_rack_screen_active',
    'passive': 't_rack_screen_passive'
}
...
class ScreenTargets(NamedTuple):
    corner_lb: Item
    corner_lt: Item
    corner_rt: Item
    corner_rb: Item
...
class ScreenFrames(NamedTuple):
    tool: Item
    ref_part: Item
    target: Item


class TVScreenMechanism:
    def __init__(self) -> None:
        ...
        self._targets: ScreenTargets = self.__create_targets()
        self._frames: ScreenFrames = self.__create_frames()
    ...
    def __create_frames(self) -> ScreenFrames:
        return ScreenFrames(
    ...
    def __create_targets(self) -> ScreenTargets:
        _items: dict[str, Item] = {
            key: self._rdk.Item(name, robolink.ITEM_TYPE_TARGET)
            for key, name in TARGETS.items()
        }

        return ScreenTargets(**_items)
